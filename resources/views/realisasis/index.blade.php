            ...
            <div class="flex flex-wrap justify-between items-center gap-4 mb-6 w-full">
                ...
                <div class="flex flex-wrap items-center justify-end gap-2 shrink-0">
                    @if(auth()->user()->role_name === 'Super Admin' || auth()->user()->hasRole('Super Admin') || auth()->user()->role_name === 'Perencanaan UID' || auth()->user()->hasRole('Perencanaan UID') || auth()->user()->hasRole('Asman Perencanaan UP3') || auth()->user()->hasRole('Asman Bidang UP3') || auth()->user()->hasRole('Bidang K3L (MSB)') || in_array(strtolower(auth()->user()->username), ['admin.k3l', 'msb.k3l']))
                        ...
                        @if(auth()->user()->hasRole('Super Admin') || auth()->user()->hasRole('Bidang K3L (MSB)') || auth()->user()->hasRole('Asman Perencanaan UP3') || auth()->user()->hasRole('Asman Bidang UP3') || strtoupper(trim((string)auth()->user()->matrix_group_id)) === 'K3L' || in_array(strtolower(auth()->user()->username), ['admin.k3l', 'msb.k3l']))
                        <a href="{{ route('realisasis.template-k3l') }}" class="w-full sm:w-auto justify-center bg-emerald-100 hover:bg-emerald-200 text-emerald-800 font-bold py-2.5 px-4 rounded-lg shadow-sm border border-emerald-200 transition-colors text-sm flex items-center whitespace-nowrap">
                            ...
                            @else
                                Template Realisasi LM - Mingguan
                            @endif
                        </a>
                        @endif
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <label class="flex items-start gap-3 p-3 rounded-lg border border-gray-200 cursor-pointer hover:bg-indigo-50 transition">
                                            ...
                                            <div>
                                                ...
                                                @else
                                                    <div class="text-sm font-semibold text-slate-700">Format Realisasi Mingguan</div>
                                                    <div class="text-[11px] text-slate-400">Sesuai dengan Template Realisasi LM - Mingguan</div>
                                                @endif
                                            </div>
                                        </label>
                                        @endif
                        ...
                        <div class="bg-white px-6 pt-5 pb-6 overflow-y-auto">
                            <div class="space-y-5">
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        <input type="number" step="0.000001" name="angka_realisasi" required class="block w-full py-2.5 px-4 pr-16 rounded-md border-slate-200 bg-slate-50/50 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200 shadow-sm text-sm text-slate-700" placeholder="0.00">
                        ...
                        <div class="bg-white px-6 pt-5 pb-6 overflow-y-auto">
                            <div class="space-y-5">
                                <div>
                                    ...
                                    <input type="number" step="0.000001" name="angka_realisasi" x-model="editForm.angka_realisasi" required class="block w-full py-2.5 px-4 rounded-md border-slate-200 bg-slate-50/50 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200 shadow-sm text-sm text-slate-700">
                                </div>
                                <div>
